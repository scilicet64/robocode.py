#
# tank.py: a Tkinter & turtle based graphics module for Python
# Version 1.2.1 - 14. 1. 2018 Allard
#
...
class Shape(object):
    ...
    def __init__(self, type_, data=None, org_image=None, name=None):
        self._type = type_
    ...
    def addChildShape(self, child, offset=(0,0)):
        self._child = child
...
#    _STANDARD_DELAY = 5
    ...
    def register_shape(self, name, shape=None, rotate_angle=None):
        ...
        if shape is None:
            ...
            if isfile(name):
                ...
                if (rotate_angle != None):
                    image = image.rotate(rotate_angle, expand=1)
                photo_image = ImageTk.PhotoImage(image)
                shape = Shape("image", data=photo_image, org_image=image, name=name)
            else:
    ...
    def remove_turtle(self,qturtle):
        titem = qturtle.turtle._item
        qturtle.turtle = None
        self._delete(titem)
        self._turtles.remove(qturtle)
    ...
    def bgpic(self, picname=None):
        ...
        if picname not in self._bgpics:
            ...
            self._bgpics[picname] = photoimage
...
class TNavigator(object):
    ...
    def __init__(self, mode=DEFAULT_MODE, startPos=Vec2D(0.0,0.0)):
        ...
        self._mode = None
        self._setmode(mode)
        TNavigator.reset(self,startPos)

    def reset(self,startPos=Vec2D(0.0,0.0)):
    ...
    def nextPostForward(self,distance, angle):
    ...
    def collision(self,allowedtypes,maxdist):
        pos= self.pos()
        collissions = []
        for t in self._screen._turtles:
            if self!=t and t.type() in allowedtypes:
                distance = t.distance(pos)
                if( distance <= maxdist):
                    collissions.append(t)
        return collissions
...
class _TurtleImage(object):
    ...
    def _setshape(self, shape):
        ...
        if self._type == "polygon" == self.shape._type:
            return
        if self._type == "image" == self.shape._type:
            return
...
class RawTurtle(TPen, TNavigator):
    ...
    def __init__(self, canvas=None,
        ...
        screen = self.screen
        TNavigator.__init__(self, screen.mode(),startPos)
        TPen.__init__(self)
        screen._turtles.append(self)
        self.drawingLineItem = screen._createline()

        if shape is None:
            shape = self.shapes[_CFG["shape"]]
        ...
        self.setundobuffer(undobuffersize)
        self._update()
    ...
    def isOncanvas(self):
        """Check if turtle is on the displayable canvas"""
        pos = self.position()
    ...
    def reset(self,startPos=(0.0,0.0)):
...
,
        ...
        0.0
        """
        TNavigator.reset(self, startPos=(0.0,0.0))
        TPen._reset(self)
        self._clear()
        self._drawturtle()
        self._update()
    ...
    def clone(self):
        ...
        q.screen = screen
        q.turtle = _TurtleImage(screen, self.turtle.shape)

        screen._turtles.append(q)
        ttype = self.turtle.shape._type
        if ttype == "polygon":
    ...
    def _polytrafo(self, poly):
        ...
        return [(p0+(e1*x+e0*y)/screen.xscale, p1+(-e0*x+e1*y)/screen.yscale)
                                                           for (x, y) in poly]
    def _imagetrafo(self, shape):
        image = shape._org_image.copy()

        while (shape._child != None):
            childimage = shape._child._org_image.rotate(shape._child.heading())
            image.paste(childimage, shape._childoffset, childimage)
            shape = shape._child

        rotimage = image.rotate(self.heading(), expand=1)
        shape._data = ImageTk.PhotoImage(rotimage)
        return shape
    ...
    def _drawturtle(self):
        ...
        if self._shown and screen._updatecounter == 0 and screen._tracing > 0:
            ...
            if ttype == "polygon":
                ...
                shape = self._imagetrafo(shape)
                screen._drawimage(titem, self._position, shape._data)
            elif ttype == "compound":
                lx, ly = self._stretchfactor
                w = self._outlinewidth
    ...
    def destroy(self):
        self.clear()
        self.screen.remove_turtle(self)
...
##############################  stamp stuff  ###############################
    ...
    def _write(self, txt, align, font, offset=(0,0)):
        """Performs the writing for write()
        """
        if (self.writeitem!=None):
            self.items.remove(self.writeitem)
            self.screen._delete(self.writeitem)
        position = self._position + offset
        item, end = self.screen._write(position, txt, align, font,
    ...
    def write(self, arg, move=False, align="left", font=("Arial", 8, "normal"), offset=(0,0)):
        ...
        if self.undobuffer:
            self.undobuffer.push(["seq"])
            self.undobuffer.cumulate = True
        end = self._write(str(arg), align.lower(), font, offset)
        if move:
            x, y = self.pos()
            self.setpos(end, y)
        if self.undobuffer:
...
class Turtle(RawTurtle):
    ...
    def __init__(self,
                 ...
                 undobuffersize=_CFG["undobuffersize"],
                 visible=_CFG["visible"],
                 startPos=Vec2D(0.0,0.0)):
        if Turtle._screen is None:
            Turtle._screen = Screen()
        if shape is None:
            shape = self.screen._shapes[_CFG["shape"]]
